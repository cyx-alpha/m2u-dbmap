...
class SleePyCoBackbone(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.training_mode in ['freezefinetune', 'scratch', 'fullfinetune', 'multimodal_pretrain']:
    ...
    def forward(self, x):
        ...
        if self.training_mode in ['pretrain']:
            out.append(c5)
        elif self.training_mode in ['scratch', 'fullfinetune', 'freezefinetune', 'multimodal_pretrain']:
            p5 = self.conv_c5(c5)
            out.append(p5)
            if self.num_scales > 1:
                p4 = self.conv_c4(c4)
